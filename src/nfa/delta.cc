...
StatePost::Moves::const_iterator::const_iterator(const StatePost& state_post): state_post_{ &state_post } {
    if (state_post_->empty()) { // No transition found, 'state_post' is an empty state post.
        is_end_ = true;
        return;
    }
...
StatePost::Moves::const_iterator::const_iterator(
    ...
    if (first_symbol_lookup_direction == Moves::FirstSymbolLookupDiretion::Forward) {
        ...
        if (state_post_it_ == state_post_end) {
            is_end_ = true;
            return;
        }
    } else { // first_symbol_lookup_direction == Moves::FirstSymbolLookupDiretion::Backward.
        auto previous_state_post_it = std::prev(state_post_end);
        while (previous_state_post_it != state_post_begin && last_symbol < previous_state_post_it->symbol) {
            --previous_state_post_it;
        }
        if (previous_state_post_it->symbol < first_symbol) {
            is_end_ = true;
            return;
        }
